...
class SunshineGPT(loader.Module):
    ...
    async def ghist(self, message):
        ...
        if user:
            ...
            prompt = f"Проанализируй следующие сообщения {user_name} и подытожи, что он обсуждал, в конце пишешь шутку о данных сообщениях и подписываешь её как Шутка от ИИ, ты не используешь символ звездочки:\n\n{chat_text}"
        else:
            title = "<b>Что сегодня обсуждали участники чата?</b>"
            prompt = f"Проанализируй следующие сообщения всех участников чата и подытожи, что обсуждали участники чата, в конце пишешь шутку о данных сообщениях и подписываешь её как Шутка от ИИ, ты не используешь символ звездочки:\n\n{chat_text}"
