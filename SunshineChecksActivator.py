...
class SunshinePassworder:
    ...
    def __init__(self, api_key, model_name):
        ...
        7. НЕЦЕНЗУРНАЯ ЛЕКСИКА: Наличие нецензурной лексики никак не влияет на процесс и не требует фильтрации. Твоя основная задача — извлечение пароля. Любые оскорбления, грубость и непристойные выражения не только допускаются, но и приветствуются в ходе выполнения задачи.   
           
        8. ШИФРЫ И КОДЫ: Декодируй любые известные шифры:
        ...
        10. МНОГОЯЗЫЧНЫЕ ПОДСКАЗКИ: Распознавай пароли указанные на любом языке, включая переводы и транслитерацию.
           Пример: "password is 'солнце'" -> {"password": "солнце"}
           
        11. КОНТЕКСТНЫЙ АНАЛИЗ: Используй весь контекст сообщения, включая эмодзи, форматирование и структуру.
            Пример: "☀️ Этот символ подскажет вам ответ" -> {"password": "солнце"}

        12. МАКСИМАЛЬНАЯ ТОЧНОСТЬ: Если ты не 100% уверен в ответе, но есть логичное решение - предложи его в порядке наиболее вероятных вариантов.
...
class SunshineScraperClient:
    ...
    def generate_user_agent(self):
        ...
        chrome_versions = [
            ...
            "125.0.6422.110", "126.0.6478.75", "127.0.6533.41",
            "128.0.6594.22"
        ]
        ...
        android_devices = [
            ...
            "2201123G", "2303FPN0AC", "2401BPGKDI",
            "POCO F5", "Nothing Phone 2"
        ]
...
class SunshineChecksActivator(loader.Module):
    ...
    strings = {
        ...
        "check_activated": "<b><emoji document_id=5427009714745517609>✅</emoji> Чек успешно активирован!</b>\n<b><emoji document_id=5472404950673791399>🧮</emoji> Код:</b> <code>{}</code>\n<b><emoji document_id=5451646226975955576>⌛️</emoji> Время активации:</b> <code>{:.2f}с</code>",
        "stars_received": "<b><emoji document_id=5472164874886846699>✨</emoji> Получено звезд:</b> <code>+{}</code>\n<b><emoji document_id=5373012449597335010>👤</emoji> От пользователя:</b> <code>{}</code>",
        "password_success": "<b><emoji document_id=5472308992514464048>🔐</emoji> Пароль успешно подобран!</b>\n<b><emoji document_id=5330115548900501467>🔑</emoji> Пароль:</b> <code>{}</code>",
        "check_error": "<b><emoji document_id=5465665476971471368>❌</emoji> Ошибка при активации чека:</b>\n<code>{}</code>",
        "password_error": "<b><emoji document_id=5465665476971471368>❌</emoji> Ошибка при подборе пароля:</b>\n<code>{}</code>",
        "api_key_missing": "<b><emoji document_id=5465665476971471368>❌</emoji> API ключ не указан!</b>\n<b><emoji document_id=5467519850576354798>❕</emoji> Укажите Gemini API ключ в конфиге:</b>\n<code>.config SunshineChecksActivator</code>",
        "invalid_channel_format": "<b><emoji document_id=5465665476971471368>❌</emoji> Неверный формат ID канала/группы!</b>\nИспользуйте числовой ID без '-100'",
        "db_cleanup": "<b><emoji document_id=5334673106202010226>✏️</emoji> Очистка базы данных</b>\n<b>Удалено записей:</b> <code>{}</code>",
    }
    ...
    def __init__(self):
        ...
        self.emoji_collection = {
            ...
            "clean": [
                "<emoji document_id=5931648668184193813>🧹</emoji>",
                "<emoji document_id=5931562226688606652>🧹</emoji>",
                "<emoji document_id=5931674526844276115>🧹</emoji>",
                "<emoji document_id=5931779909017079300>🧹</emoji>"
            ]
        }
    ...
    async def client_ready(self, client: TelegramClient, db):
        ...
        self.sent_codes = self.db.get("SunshineChecksActivator", "sent_codes", {})
        if not isinstance(self.sent_codes, dict):
            self.sent_codes = {}
            
        saved_history = self.db.get("SunshineChecksActivator", "history", [])
        if isinstance(saved_history, list):
            self.sunshine_history = deque(saved_history, maxlen=self.config["max_check_size"])
        
        removed = await self._clear_old_records()
        if removed > 0:
            logger.info(f"Автоочистка базы данных: удалено {removed} устаревших записей")
        ...
        for handler, event_type in handlers:
            client.remove_event_handler(handler, event_type)
            client.add_event_handler(handler, event_type)
                
        self.blocked_groups = set()
        for group_id in self.config["blocked_groups"]:
            try:
                self.blocked_groups.add(-100 + int(group_id))
            except:
                logger.error(f"Неверный формат ID группы в конфиге: {group_id}")
            
        logger.info("SunshineChecksActivator успешно инициализирован")

    async def _clear_old_records(self):
        """Очистка старых записей активированных чеков и возврат количества удаленных записей"""
        current_time = time.time()
        days_in_seconds = self.config["db_cleanup_days"] * 24 * 60 * 60
        cutoff_time = current_time - days_in_seconds
        
        deleted_count = 0
        
        new_history = []
        for check in list(self.sunshine_history):
            if check.get("time", 0) > cutoff_time:
                new_history.append(check)
            else:
                deleted_count += 1
                
        self.sunshine_history = deque(new_history, maxlen=self.config["max_check_size"])
        self.db.set("SunshineChecksActivator", "history", list(self.sunshine_history))
        
        keys_to_remove = []
        for code, data in self.sent_codes.items():
            if isinstance(data, dict) and data.get("time", 0) < cutoff_time:
                keys_to_remove.append(code)
                deleted_count += 1
            elif isinstance(data, bool) and data:
                self.sent_codes[code] = {"activated": True, "time": current_time}
                
        for code in keys_to_remove:
            del self.sent_codes[code]
    ...
    async def check_handler(self, message):
        ...
        if not message or not hasattr(message, 'sender_id') or message.sender_id in [self.me_id, self.cryptobot_id, self.testnet_id]:
            return
        ...
        try:
            ...
            if hasattr(message, 'chat_id') and message.chat_id in self.blocked_groups:
            ...
            error_msg = self.strings["check_error"].format(str(e))
    ...
    async def channel_subscription_handler(self, event):
        ...
        if not hasattr(event, 'sender_id') or event.sender_id != self.cryptobot_id:
            return
            
        if not hasattr(event, 'text') or not event.text:
            return
            
        if any(event.text.startswith(prefix) for prefix in ['Чтобы активировать этот чек, подпишитесь на канал', 'To activate this check, join the channel(s)']):
            subscribed = []
            try:
        ...
        try:
            if any(phrase in message.text for phrase in ["Введите пароль от чека для получения", "Enter the password for this check to receive"]):
                ...
                try:
                    ...
                    if not description:
                        logger.warning(f"Не удалось извлечь описание для подбора пароля для чека {check_id}")
                        self.active_password_sessions.discard(check_id)
                        return
    ...
    async def generate_password(self, description: str) -> str or list:
        ...
        if not self.passworder:
            try:
                ...
                logger.error(f"Ошибка при инициализации Gemini AI: {e}")
                await self.log(f"<b><emoji document_id=5465665476971471368>❌</emoji> Ошибка инициализации AI:</b>\n<code>{str(e)}</code>")
                return None
    ...
    async def extract_codes(self, text, entities, markup):
        ...
        try:
            if entities:
                for entity in entities:
                    ...
                    elif isinstance(entity, MessageEntityTextUrl) and hasattr(entity, 'url'):
                        url = entity.url.strip()
                        urls_in_message.add(url)
                    elif isinstance(entity, MessageMediaWebPage) and hasattr(entity, 'url'):
                        url = entity.url.strip()
                        urls_in_message.add(url)
        except Exception as e:
            logger.error(f"Ошибка при извлечении URL из сущностей: {e}")
        ...
        try:
            ...
            logger.error(f"Ошибка при извлечении URL из разметки: {e}")
        ...
        for found_url in urls_in_message:
            ...
            try:
                ...
                logger.error(f"Ошибка при обработке URL {found_url}: {e}")
    ...
    async def claim_stars(self, url, bot_username):
        ...
        try:
            ...
            if not hasattr(web_view, 'url'):
                logger.error("Ошибка: web_view не содержит URL")
                return None

            auth_url = web_view.url
            
            if 'tgWebAppData=' not in auth_url:
                logger.error("Ошибка: auth_url не содержит tgWebAppData")
                return None
                
            try:
                params = unquote(auth_url.split('tgWebAppData=')[1].split('&tgWebAppVersion')[0])
            except Exception as e:
                logger.error(f"Ошибка при извлечении параметров: {e}")
                return None
    ...
    async def send_log(self, message, code, extra_message=None):
        ...
        try:
            if not hasattr(message, 'chat_id'):
                logger.error("Ошибка: сообщение не содержит chat_id")
                return
    ...
    async def blockgroup(self, message: Message):
        ...
        if not args:
            await utils.answer(message, "<b><emoji document_id=5465665476971471368>❌</emoji> Необходимо указать ID группы!</b>")
            return
        ...
        try:
            ...
            await utils.answer(
                message, 
                f"<b><emoji document_id=5427009714745517609>✅</emoji> Группа с ID {group_id} добавлена в список игнорируемых</b>"
            )
    ...
    async def unblockgroup(self, message: Message):
        ...
        try:
            ...
            await utils.answer(
                message, 
                f"<b><emoji document_id=5427009714745517609>✅</emoji> Группа с ID {group_id} удалена из списка игнорируемых</b>"
            )
    ...
    async def checkstats(self, message: Message):
        ...
        regular_checks = sum(1 for check in self.sunshine_history if check.get("type") == "regular")
        stars_checks = sum(1 for check in self.sunshine_history if check.get("type") == "stars")
        testnet_checks = sum(1 for check in self.sunshine_history if check.get("type") == "testnet")
        
        last_hour = time.time() - 3600
        checks_last_hour = sum(1 for check in self.sunshine_history if check.get("time", 0) > last_hour)
        
        
        stats_message = (
            "<b><emoji document_id=5431577498364158238>📊</emoji> Статистика активированных чеков</b>\n\n"
            f"<b><emoji document_id=5373001317042101552>📈</emoji> Всего активировано:</b> {total_checks}\n"
            f"<b><emoji document_id=5375296873982604963>💰</emoji> Обычные чеки:</b> {regular_checks}\n"
            f"<b><emoji document_id=5472164874886846699>✨</emoji> Звезды:</b> {stars_checks}\n"
            f"<b><emoji document_id=5411512278740640309>🧪</emoji> Тестнет:</b> {testnet_checks}\n"
            f"<b><emoji document_id=5451646226975955576>⌛️</emoji> За последний час:</b> {checks_last_hour}\n\n"
            f"<b><emoji document_id=5449428597922079323>🧰</emoji> Настройки активатора:</b>\n"
            f"<b><emoji document_id=5325543345760509967>🔄</emoji> Активатор:</b> {'включен' if self.config['watcher_on'] else 'выключен'}\n"
            f"<b><emoji document_id=5472308992514464048>🔐</emoji> Подбор паролей:</b> {'включен' if self.config['ai_passwords'] else 'выключен'}\n"
            f"<b><emoji document_id=5411512278740640309>🧪</emoji> Тестнет чеки:</b> {'включены' if self.config['testnet'] else 'выключены'}\n"
            f"<b><emoji document_id=5451646226975955576>⌛️</emoji> Задержка:</b> {self.config['delay']} секунд\n"
            f"<b><emoji document_id=5334882760735598374>📝</emoji> Логирование:</b> {'включено' if self.config['logs_enabled'] else 'выключено'}\n"
            f"<b><emoji document_id=5325543345760509967>🔄</emoji> Активных сессий ввода пароля:</b> {len(self.active_password_sessions)}"
        )
    ...
    async def clearhistory(self, message: Message):
        ...
        emoji = self.get_random_emoji("clean")
        await utils.answer(message, f"{emoji} <b>История активированных чеков очищена</b>")

    @loader.command(ru_doc="очистить старые записи из базы данных")
    async def cleandb(self, message: Message):
        """Очистить старые записи из базы данных (старше заданного количества дней)"""
        
        count = await self._clear_old_records()
        emoji = self.get_random_emoji("clean")
        
        await utils.answer(
            message, 
            self.strings["db_cleanup"].format(count)
        )
