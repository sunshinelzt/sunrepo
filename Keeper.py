...
class KeeperMod(loader.Module):
    ...
    async def save_media(self, message):
        ...
        try:
            ...
            if not media_bytes:
                logger.error("Не удалось скачать медиа")
                return False
    ...
    def _make_caption(self, message):
        ...
        if sender:
            ...
            caption += f"<b>От:</b> <a href='tg://user?id={sender_id}'>{display_name}</a>\n"
    ...
    def _get_state(self):
        """Получает текущее состояние автосохранения"""
    ...
    async def watcher(self, m):
        ...
        if not m or not self._get_state():
            return
