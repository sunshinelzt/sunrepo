...
class iibotMod(loader.Module):
    ...
    def __init__(self):
        ...
        self.active_chats = set()  # Включенные чаты
        self.reply_chance = 0  # Шанс ответа (0 = всегда)
    ...
    async def randomicmd(self, m: types.Message):
        """Установить шанс ответа (0 - всегда, N - 1 к N)"""
    ...
    async def watcher(self, m: types.Message):
        ...
        if chat_id not in self.active_chats:
            return
        if user_id == (await m.client.get_me()).id or user_id in self.config["blacklist"]:
            return
    ...
    def generate_random_response(self):
        """Генерирует случайную глупую фразу"""
        phrases = [
            "Хм... даже не знаю 🤔",
